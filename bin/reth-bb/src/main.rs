...
impl PayloadValidator<BbPayloadTypes> for BbEngineValidator {
    ...
    fn convert_payload_to_block(
        ...
        // A big block is assembled by folding every segment into the last one, so an empty
        // segment list has no block to fold into and would otherwise panic downstream.
        if payload.env_switches.is_empty() {
            return Err(NewPayloadError::Other(
                "big block payload must carry at least one execution segment".into(),
            ));
        }
...
mod tests {
    ...
    /// An empty segment list has no block to fold the remaining segments into, so it must be
    /// rejected rather than reaching the assembly path.
    #[test]
    fn rejects_big_block_payload_without_segments() {
        let validator = BbEngineValidator {
            inner: EthereumEngineValidator::new(std::sync::Arc::new(
                reth_chainspec::ChainSpec::default(),
            )),
        };

        let payload = BigBlockData {
            env_switches: Vec::new(),
            prior_block_hashes: Vec::new(),
            block_number: 1,
            merged_block_access_list: None,
        };

        let err = PayloadValidator::<BbPayloadTypes>::convert_payload_to_block(&validator, payload)
            .expect_err("empty segment list must be rejected");

        assert!(
            err.to_string().contains("at least one execution segment"),
